...
const Timer = () => {
  ...
  const startTimer = useCallback(() => {
    ...
    if (!start) {
      ...
      started.current = setInterval(() => {
        setExtraTime(startedTime - Date.now() + selectTime * 1000);
      }, 10);
    } else {
      if (
  ...
  const Pause = useCallback(() => {
    ...
    if (!pauseAndContinue) {
      ...
      startedForTimer.current = setTimeout(completeTimer, extraTime);

      started.current = setInterval(() => {
        setExtraTime(startedTime - Date.now() + extraTime);
      }, 10);
    }
        ...
        <View style={styles.clockSection}>
          ...
          {start ? (
            <View style={styles.parent}>
              <Text style={styles.child}>{UseParseClock(extraTime)}</Text>
            </View>
          ) : (
            <TimePicker
