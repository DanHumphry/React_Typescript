...
function ReducerSample() {
  ...
  const setText = () => dispatch({ type: 'SET_TEXT', text: state.text === 'hello' ? 'bye' : 'hello' }); // text 를 넣지 않으면 에러 발생
  const setColor = () => dispatch({ type: 'SET_COLOR', color: state.color === 'red' ? 'yellow' : 'red' }); // orange 를 넣지 않으면 에러 발생
